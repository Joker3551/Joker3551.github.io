;(function() {
	...
	// флаг ручного размещения кораблей
	let isHandlerPlacement = false;

	// флаг ведения морского боя
	let isHandlerController = false;

	// флаг блокирующий действия игрока во время игры компьютера
	let compShot = false;
	...
	class Field {
		...
		constructor(field) {
			...
			// пустой объект, для даннных по каждому кораблю
			this.squadron = {};


			// массив с кординатами кораблей и не только
			this.matrix = [];

			//координаты сторон рамки игрового поля
	...
	class Placement {
		...
		onMouseDown(e) {
			...
			// проверка, что нажатие произошло над кораблём
			...
			// проверка, что корабль находится на поле игрока
		...
		onMouseMove(e) {
			...
			if (!this.clone) {
				...
				// количество палуб у перемещаемого корабля
				...
				// сдвиг курсора по координатам X и Y
			...
			// проверка, что клон находится в пределах игрового поля
			if (left >= Placement.FRAME_COORDS.left - 14 && right <= Placement.FRAME_COORDS.right + 14 && top >= Placement.FRAME_COORDS.top - 14 && bottom <= Placement.FRAME_COORDS.bottom + 14) {
				...
				// если клон находится в пределах игрового поля
				// подсвечиваем его зелёным цветом
				...
				if (!result) {
					...
					// если в соседних клетках находятся ранее установленные корабли
					// подсвечиваем его красным цветом
					this.clone.classList.remove('success');
					this.clone.classList.add('unsuccess');
				}
			} else {
				
				// если клон находится за пределами игрового поля
				// подсвечиваем его красным цветом
				this.clone.classList.remove('success');
				this.clone.classList.add('unsuccess');
			}
	...
	class Controller {
		...
		init() {
			...
			// Рандомно выбираем кто будет ходить
			...
			if (this.player === human) {
				compShot = false;
				this.text = 'Вы начинаете';
			} else {
				compShot = true;
				this.text = 'Начинает компьютер';
				// выстрел компьютера
		...
		setCoordsAroundHit(x, y, coords) {
			...
			// второе попадание
				...
				// вычисляем направление расположение корабля
		...
		setUselessCell(e) {
			...
			// устанавливаем маркер пустой клетки
			if (check) {
		...
		showIcons(opponent, [x, y], iconClass) {
			...
			// задержка при формировании иконок промаха и попадания
		...
		miss(x, y) {
			...
			if (this.player === human) {
				text = 'Промах. Стреляет компьютер.';
		...
		hit(x, y) {
			...
			text = (this.player === human) ? 'Вы попали. Ваш выстрел.' : 'Компьютер попал. Выстрел компьютера';
			setTimeout(() => Controller.showServiceText(text), 400);
